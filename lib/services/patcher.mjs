export class Patcher {
  ...
  _updateCommenterRow(infoBox, row, item, existingCommenter = null) {
    ...
    if (infoBox.editable) {
      ...
      input.id = 'itembox-field-commenter-input';
      input.className = 'value';
      input.placeholder = 'Enter commenter name';
  ...
  _findCommenterAnchorRow(infoBox) {
    return this._findInfoFieldRow(infoBox, 'court')
      || this._findInfoFieldRow(infoBox, 'extra')
      || this._findInfoFieldRow(infoBox, 'date')
      || null;
  }
  ...
  _commenterValuesEqual(a, b) {
    ...
    return String(a?.firstName || '').trim() === String(b?.firstName || '').trim()
      && String(a?.lastName || '').trim() === String(b?.lastName || '').trim();
  }
  ...
  async _saveCommenterFromInput(item, rawValue, useLiteralName = false) {
    try {
      ...
      try { Zotero.debug(`[IndigoBook CSL-M] commenter row saved: item=${String(item.id)} value=${String(rawValue)}`); } catch (e) {}
    } catch (e) {
      try { Zotero.logError(e); } catch (_) {}
      try { Zotero.debug(`[IndigoBook CSL-M] commenter row save failed: ${String(e)}`); } catch (_) {}
    }
  }
